...
public class MigrationCalculator {
	...
	public double calculateMigrationTime(MigrationMessage msg, double currentMigrationClock){
	...
	public double calculateMigrationSize(MigrationMessage msg, double currentMigrationClock, double desireTime){
		double migrationTime = 0;
		/*double dataKB = msg.getDataSizeKB();*/
		double totalSentKB = 0;
		double bwAtTimeKB = 0;
		
		while(migrationTime < desireTime){
			//Check if the time exceeded the limit already
			if(currentMigrationClock >= Environment.migrationTimeLimit){
				return Double.MIN_VALUE;
			}			
			...
			double roundSend, usedTime;
			if(isReachedTimeWitninInterval(migrationTime, desireTime, intervalFraction)){
				double leftoverTime = desireTime - migrationTime;
				roundSend = leftoverTime * bwAtTimeKB;
				usedTime = leftoverTime;
			}
			else{
				roundSend = bwAtTimeKB * intervalFraction;
				usedTime = intervalFraction;
	...
	private boolean isReachedTimeWitninInterval(double usedTime, double desireTime, double intervalFraction){
		double time = usedTime + intervalFraction;
		if(time >= desireTime){
			return true;
		}
		return false;
